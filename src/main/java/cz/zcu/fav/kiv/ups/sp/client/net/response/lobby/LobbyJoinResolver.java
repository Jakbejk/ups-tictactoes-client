 ...
 * @project UPS-client
 */
public class LobbyJoinResolver implements IResolver {

    private final static Logger logger = LogManager.getLogger(LobbyJoinResolver.class);

    @Override
    public void resolve(ResponseMessage responseMessage) throws InvalidResponseException {
        logger.debug("LOBBY_JOIN request: " + responseMessage + " received.");
        ...
        if (responseMessage.getResponse() == ResponsePrefix.OK) {
            Platform.runLater(() -> lobbyController.onLobbyJoinSuccess(responseMessage));
        } else if (responseMessage.getResponse() == ResponsePrefix.ERROR) {
            Platform.runLater(() -> lobbyController.onLobbyJoinError(responseMessage));
        } else {
